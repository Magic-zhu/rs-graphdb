  ...
  <div class="cm-editor-container">
    ...
    <div class="flex items-center justify-between mb-2">
      <div class="flex items-center gap-2">
        <span class="text-sm font-medium text-gray-300">查询编辑器</span>
        <span v-if="currentLine" class="text-xs text-gray-500">行 {{ currentLine }}</span>
        <span v-if="currentCol" class="text-xs text-gray-500">列 {{ currentCol }}</span>
      </div>
      <div class="flex items-center gap-2">
        <button
    ...
    <!-- Editor Container -->
    <div ref="editorContainer" class="editor-wrapper mb-2"></div>

    <!-- Action Buttons -->
    <div class="flex items-center gap-2 mb-2">
      <button
        @click="executeQuery"
        :disabled="loading || !getQuery()"
...
// Simple query keyword completion
const graphCompletions = [
...
onMounted(() => {
  const parent = editorContainer.value
  if (!parent) return
    ...
    extensions: [
      ...
      EditorView.updateListener.of((update: any) => {
        if (update.selectionSet) {
          ...
          currentCol.value = pos - line.from + 1
        }
      }),
    ],
  })
...
function completionSource(context: any) {
  ...
  const filtered = graphCompletions.filter((c) =>
...
function setQuery(query: string) {
  const transaction = editorView.value?.state.update({
    changes: {
      from: 0,
      to: editorView.value?.state.doc.length || 0,
      insert: query,
    },
  })
  if (transaction) {
...
async function executeQuery() {
  ...
  try {
    ...
    queryHistoryStore.addQuery(query, true, undefined, executionTime)
  } catch (err) {
    const errorMessage = err instanceof Error ? err.message : '未知错误'
    showMessage(`查询失败: ${errorMessage}`, 'error')
    queryHistoryStore.addQuery(query, false, undefined, undefined, errorMessage)
  } finally {
    loading.value = false
  }
}
...
function formatQuery() {
  ...
  // Simple formatting: add proper spacing and line breaks
...
.editor-wrapper :deep(.cm-lineNumbers) {
...
.editor-wrapper :deep(.cm-tooltip) {
  @apply border border-gray-600 rounded-lg;
}

.editor-wrapper :deep(.cm-tooltip-autocomplete) {
